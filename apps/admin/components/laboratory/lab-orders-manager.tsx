...
export function LabOrdersManager({ canWrite }: Readonly<{ canWrite: boolean }>) {
  ...
  const columns: ColumnDef<LabOrderListItem>[] = [
    { id: "orderedAt", accessorKey: "orderedAt", header: "Orden", cell: ({ row }) => <div className="table-module-cell"><strong>#{row.original.id}</strong><span>{formatDateTime(row.original.orderedAt)}</span></div> },
